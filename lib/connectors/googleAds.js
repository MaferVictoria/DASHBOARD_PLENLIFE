...
export async function getGoogleMonthly() {
  ...
  const query = `
    SELECT segments.month, metrics.cost_micros, metrics.conversions_value
    FROM campaign
    WHERE campaign.advertising_channel_type = 'PERFORMANCE_MAX'
  ...
  try {
    ...
    return months.map((m) => ({ month: m, spend: null, sales: null }));
  }
  ...
  rows.forEach((row) => {
    ...
    if (!byMonth[label]) byMonth[label] = { costMicros: 0, conversionsValue: 0 };
  ...
  return months.map((month) => {
    const bucket = byMonth[month] || { costMicros: 0, conversionsValue: 0 };
    return {
      month,
