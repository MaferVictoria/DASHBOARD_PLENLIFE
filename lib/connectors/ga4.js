...
function normalizePrivateKey(raw) {
  ...
  if (!key.startsWith('-----BEGIN PRIVATE KEY-----') || !key.endsWith('-----END PRIVATE KEY-----')) {
    throw new Error(
      `GA4 auth: GOOGLE_ANALYTICS_PRIVATE_KEY no tiene el formato esperado — debe empezar con ` +
        `"-----BEGIN PRIVATE KEY-----" y terminar con "-----END PRIVATE KEY-----" (largo actual: ` +
        `${key.length} caracteres, empieza con "${key.slice(0, 20)}..."). Vuelve a copiar el valor ` +
        `de "private_key" del archivo .json de la cuenta de servicio — solo lo que está ENTRE las ` +
        `comillas, sin incluirlas.`
    );
  }
