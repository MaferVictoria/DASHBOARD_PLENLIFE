...
export async function getEngagementKpis(start, end) {
  ...
  const report = await runGA4Report({ metrics: [...METRICS_8, 'newUsers'], since: start, until: end });
  const rows = parseReport(report);
  const r = rows[0] || {
    totalUsers: 0,
    newUsers: 0,
...
//
// BUG FIXED (25 ago 2026): this used to be '2015-01-01', which the GA4
// Data API itself rejects — it enforces a hard floor of 2015-08-13 for
// ANY property's start_date, regardless of when that property was created
// (confirmed by the API's own error message). '2015-01-01' was always an
// odd choice anyway: GA4 as a product didn't exist before October 2020, so
// no property could have real GA4 data that far back — this date just
// needs to safely predate GA4's actual launch, which 2015-08-14 does with
// room to spare.
const GA4_HISTORY_START = '2015-08-14';

// Trailing 12 calendar months (including the current one) — used ONLY by
// the two monthly CHARTS (points 3/4 of the Aug 2026 change request: they
// were showing every month since this GA4 property started tracking,
// which with ~3 years of history renders as a dense wall of bars that
// reads more like a weekly chart than a monthly trend line. The monthly
// DETAIL TABLE below intentionally keeps the full GA4_HISTORY_START range,
// since its year filter needs every past year available to pick from.
...
export async function getMonthlyDetailTable() {
  ...
  return rowsTo8Metrics(rows, 'yearMonth').map((row, i) => ({
    ...row,
    key: formatYearMonth(rows[i].yearMonth),
    monthKey: rows[i].yearMonth, // "202602" etc — sorts correctly chronologically as a plain string, unlike the display label ("Feb 2026")
    year: Number(rows[i].yearMonth.slice(0, 4)),
  }));
}
