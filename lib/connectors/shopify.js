...
export async function getShopifyTopStatesByMonth(start, end) {
  ...
  const totals = {}; // { [state]: { sales, orders } }
  orders.forEach((o) => {
    ...
    if (!totals[state]) totals[state] = { sales: 0, orders: 0 };
    totals[state].sales += sales;
    totals[state].orders += 1;
  });

  const states = Object.entries(totals)
    .map(([state, t]) => ({ state, sales: round(t.sales), orders: t.orders }))
    .sort((a, b) => b.sales - a.sales)
    .slice(0, 5);

  return [{ label: `${start} → ${end}`, states }];
}
...
export async function getShopifyTopProductsByMonth(start, end) {
  ...
  const totals = {}; // { [product]: { sales, units } }
  orders.forEach((o) => {
    o.lineItems.edges.forEach(({ node: item }) => {
      const revenue = netAmountFromGross(Number(item.discountedTotalSet.shopMoney.amount), o.taxesIncluded);
      const units = Number(item.quantity) || 0;
      if (!totals[item.title]) totals[item.title] = { sales: 0, units: 0 };
      totals[item.title].sales += revenue;
      totals[item.title].units += units;
    });
  });

  const products = Object.entries(totals)
    .map(([product, t]) => ({ product, sales: round(t.sales), units: t.units }))
    .sort((a, b) => b.sales - a.sales)
    .slice(0, 5);

  return [{ label: `${start} → ${end}`, products }];
}
