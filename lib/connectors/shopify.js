...
async function fetchOrdersInRange(since, until, { maxPages }) {
  ...
  const clean = orders.filter((o) => !o.cancelledAt && !o.test);
...
// NOTE: top states/products used to be computed here from live Shopify
// orders (buildTopStatesByMonth/buildTopProductsByMonth + a "last 3 months
// of orders" fetch). That's now replaced by lib/connectors/googleSheets.js,
// which reads Plenlife's manually-maintained sales export instead — see
// that file for why. Removed here rather than left dead so nobody has two
// competing implementations to keep in sync.
...
export async function getShopifyMonthly() {
  ...
  orders.forEach((o) => {
    ...
    byMonth[month].sales += Number(o.currentSubtotalPriceSet.shopMoney.amount);
...
// Top estados / top productos moved to lib/connectors/googleSheets.js.
