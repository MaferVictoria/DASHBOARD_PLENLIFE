...
//
// 4. The Admin API funnel data (page views / add to cart / checkout info) is
//    NOT available here — Shopify Orders don't carry session-level funnel
//    data. That needs Shopify's Analytics API (limited availability) or GA4.
//    getShopifyFunnel() below is still mock until you wire one of those up.
...
function buildTopCitiesByMonth(orders, monthLabels) {
  const byMonth = {}; // { [month]: { [city]: sales } }
  orders.forEach((o) => {
    const month = monthLabelFor(o.createdAt);
    if (!monthLabels.includes(month)) return;
    const city = o.shippingAddress?.city || 'Sin ciudad';
    const sales = Number(o.currentSubtotalPriceSet.shopMoney.amount);
    byMonth[month] = byMonth[month] || {};
    byMonth[month][city] = (byMonth[month][city] || 0) + sales;
  });

  return monthLabels.map((month) => {
    const cities = Object.entries(byMonth[month] || {})
      .map(([city, sales]) => ({ city, sales: round(sales) }))
...
export async function getShopifyMonthly() {
  ...
  let orders;
  try {
    orders = await fetchOrdersInRange(`${now.getFullYear()}-01-01`, toISODate(now), { maxPages: 60 });
  } catch (err) {
...
// Real version: aggregates actual Shopify orders by shipping city, grouped
// by month (last 3 months). Do NOT pull city data from Meta or Google — they
// don't have it.
export async function getShopifyTopCitiesByMonth() {
  ...
  const monthLabels = monthsSoFar(new Date()).slice(-3);
  const orders = await getRecentOrdersOnce();
  return buildTopCitiesByMonth(orders, monthLabels);
}

// Real version: aggregates actual Shopify order line items by product title,
// grouped by month (last 3 months). Do NOT pull product data from Meta or
...
export async function getShopifyFunnel(start, end) {
  // Still mock — see caveat #4 at the top of this file. This needs Shopify
  // Analytics or GA4, not the Orders API.
