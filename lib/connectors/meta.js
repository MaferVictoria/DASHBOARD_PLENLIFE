...
export async function getMetaMonthly() {
  ...
  // instead of failing the whole chart. Pulls both spend and purchase value
  // so the monthly chart can show gasto vs. ventas generadas together.
  const results = await Promise.all(
    months.map(async ({ label, start, end }) => {
      try {
        ...
        return { month: label, spend, sales };
      } catch (err) {
        console.error(`[meta connector] Failed to fetch ${label}:`, err.message);
        return { month: label, spend: null, sales: null }; // null = "could not load", not "zero"
      }
    })
  );
...
async function fetchAdLevelInsights(start, end) {
  ...
  url.searchParams.set('fields', 'ad_id,ad_name,spend,clicks,actions,action_values');
