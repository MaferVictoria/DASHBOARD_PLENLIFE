...
export function computeBlended(metaTotals, googleTotals, shopifyTotals) {
  ...
  // null (not 0) when there are no new customers to divide by — "$0 CAC"
  // would misleadingly read as "free acquisition" instead of "no data".
  const blendedCac = newCustomers > 0 ? totalSpend / newCustomers : null;
