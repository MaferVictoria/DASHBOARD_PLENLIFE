...
function mock8MetricsRow(rnd) {
  ...
  const visitantes = round(sesiones * (0.65 + rnd() * 0.2));
...
export function mockChannelBreakdown(start, end) {
  ...
  const sourceTable = [...donut].sort((a, b) => b.sessions - a.sessions).map((d) => ({ source: d.name, ...d }));
...
// GA4's two monthly charts always show full history — not scoped to the
// selected date filter (see caveat in lib/connectors/ga4.js) — so these
// mocks use a fixed 14-month window instead of monthsSoFar().
function mockGa4MonthKeys() {
  const now = new Date();
  const months = [];
  for (let i = 13; i >= 0; i--) {
    const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
    months.push({ monthKey: `${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, '0')}`, label: `${GA4_MONTH_LABELS[d.getMonth()]} ${d.getFullYear()}` });
  }
  return months;
}

export function mockMonthlyVisitorsSessions() {
  return mockGa4MonthKeys().map(({ monthKey, label }, i) => {
...
export function mockMonthlyDetailTable() {
  return mockGa4MonthKeys().map(({ monthKey, label }, i) => {
    ...
    return { key: label, ...mock8MetricsRow(rnd) };
  });
}
