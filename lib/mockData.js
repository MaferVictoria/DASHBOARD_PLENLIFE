...
export function mockTopStatesByMonth(referenceDate = new Date()) {
  const months = monthsSoFar(referenceDate).slice(-2); // current + previous month
  return months.map((month) => {
    const rnd = rngFor('states', month);
    const states = STATES
      .map((state) => ({ state, sales: round(9000 + rnd() * 60000) }))
...
const CREATIVE_CONCEPTS = [
  'BR+ Chocolate — Antes/Después',
  'Gomitas Calcio — Testimonial',
  'Multivitamínico — Rutina matutina',
  'BR+ Vainilla — UGC cliente real',
  'Kit Trío Gomitas — Oferta 20%',
  'Proteína Keto — Comparativa',
...
export function mockTopCreatives(start, end, limit = 5) {
  ...
  const rows = CREATIVE_CONCEPTS.map((concept, i) => {
    ...
    const clicks = round(purchases * (18 + seed() * 22));
    return {
      adId: `mock-${i}`,
      adName: concept,
